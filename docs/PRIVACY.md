...
| Private test access | GitHub's device authorization flow, approved by you on GitHub. Only invited accounts can access private repositories. |
| Wireless ADB | A local authenticated connection to the headset's Android debugging service. This is distinct from GitHub access. |
...
App-private data includes preferences, ADB pairing identity, inventory/presentation state and any private-test GitHub credentials. GitHub credentials are encrypted using Android Keystore-backed AES-GCM storage. Do not mistake encryption for a guarantee against a compromised rooted device.
