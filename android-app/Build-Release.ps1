...
$output = Join-Path $releaseDirectory 'Quest-Home-Switcher-v1.3.0.apk'
...
try {
    ...
    if ($LASTEXITCODE -ne 0 -or $badging -notmatch "package: name='dev\.codex\.questhomeswitcher'") {
        throw 'The signed APK has an unexpected package ID.'
    }
    if ($badging -notmatch "versionCode='12'" -or $badging -notmatch "versionName='1\.3\.0'") {
